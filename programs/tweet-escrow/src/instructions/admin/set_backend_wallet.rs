...
#[derive(Accounts)]
pub struct SetBackendWalletCtx<'info> {
    #[account(mut)]
...
pub struct SetBackendWalletParams {
    pub new_backend_wallet: Pubkey,
}
...
pub fn handler<'info>(
    ...
    msg!(">>> set backend wallet");

    let escrow_config = ctx.accounts.escrow_config.as_mut();
    escrow_config.backend_wallet = params.new_backend_wallet;
